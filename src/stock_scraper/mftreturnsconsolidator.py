...
def _warn(msg: str) -> None:
    print(f"[mftreturnsconsolidator] {msg}")
...
def _load_rows(path: str) -> tuple[list[str], list[list[Any]]]:
    ...
    if not rows:
        raise RuntimeError(f"No rows found in input workbook: {path}")
    ...
    if header_row_idx >= len(rows):
        raise RuntimeError(f"Could not locate a valid header row in workbook: {path}")
...
def _build_col_index(headers: list[str]) -> dict[str, int]:
    ...
    return {
        ...
        "nav": pick("NAV", "Nav"),
        "aum": pick("AUM", "AUM (Cr)", "AUM (in Cr)", "AUM (Crore)"),
        "ter": pick("TER", "TER (%)"),
        "r1": pick("1 Year Rtn", "1 Yr Rtn (%)"),
        "r3": pick("3 Year Rtn", "3 Yrs Rtn (%)"),
        "r5": pick("5 Year Rtn", "5 Yrs Rtn (%)"),
        "r10": pick("10 Year Rtn", "10 Yrs Rtn (%)"),
...
def _log_missing_columns(label: str, idx_map: dict[str, int], required_keys: list[str]) -> None:
    missing = [k for k in required_keys if idx_map.get(k, -1) < 0]
    if missing:
        _warn(f"{label} missing columns (aliases did not match): {', '.join(missing)}")
...
def consolidate_mft_returns(
    ...
    _log_missing_columns("trailing", t_idx, ["scheme", "category", "nav", "r1", "r3", "r5", "r10"])
    _log_missing_columns("risk", r_idx, ["scheme", "aum", "ter"])
    ...
    for row in risk_rows:
        ...
        if not key:
            continue
        risk_by_scheme[key] = {
            "TER": _value(row, r_idx["ter"]),
            "AUM": _value(row, r_idx["aum"]),
    ...
    # Freeze the header row so data scrolls under it.
